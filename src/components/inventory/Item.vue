        ...
        <div ref="tooltipRef">
            ...
            <div v-if="item.base_damage">
                <div v-if="item.base_damage.mindam && item.base_damage.maxdam">
                    One Hand Damage: {{ item.base_damage.mindam }}-{{
                        item.base_damage.maxdam
                    }}
                </div>
                <div
                    v-if="
                        item.base_damage.twohandmindam &&
                        item.base_damage.twohandmaxdam
                    "
                >
                    Two Hand Damage: {{ item.base_damage.twohandmindam }}-{{
                        item.base_damage.twohandmaxdam
                    }}
                </div>
            </div>
            ...
            <div>
                <div
                    v-for="(
                        stat, idx
                    ) in item.displayed_combined_magic_attributes"
                    :key="idx"
                    class="blue"
            ...
            <div v-if="item.total_nr_of_sockets" class="blue">
...
export default {
    ...
    computed: {
        itemClass() {
            let clazz = `${this.clazz ? this.clazz : 'item'} w-${
                this.item.inv_width
            } h-${this.item.inv_height}`
        ...
        innerClass() {
            let clazz = `${this.clazz ? this.clazz : 'inner'} w-${
                this.item.inv_width
            } h-${this.item.inv_height}`
            return clazz
        },
    ...
    methods: {
        socketStyle(idx) {
            ...
            const countX = Math.max(
                1,
                Math.ceil(this.item.total_nr_of_sockets / this.item.inv_height)
            )
            ...
            return {
                transform: `translateX(${
                    cellSize *
                    (((i + 0.5) * this.item.inv_width) / countX - 0.5)
                }px) translateY(${
                    cellSize *
                    (((j + 0.5) * this.item.inv_height) / countY - 0.5)
                }px)`,
                top: `0`,
                left: `0`,
            }
        ...
        itemName(item) {
            ...
            if (item.quality === d2s.Quality.Magic && item.magic_prefix) {
                let magic_prefix_name = constants.magic_prefixes[
                    item.magic_prefix
                ]
            ...
            if (item.quality === d2s.Quality.Magic && item.magic_suffix) {
                let magic_suffix_name = constants.magic_suffixes[
                    item.magic_suffix
                ]
