...
function getPaletteIdxForColor(color) {
  ...
  if (paletteIdxToColorMap[lookupKey] != undefined) {
    ...
    window.palettes["ACT1"].every( (paletteColor, idx) => {
      let score = Math.pow(color.r - paletteColor[0], 2) + Math.pow(color.g - paletteColor[1], 2) + Math.pow(color.b - paletteColor[2], 2)
  ...
  if (version == 31) {
    ...
    for (let t = 0; t < bch; t++) {
      for (let s = 0; s < bcw; s++, idx += 16) {
        ...
        let da = ( sprite[idx + 6] | sprite[idx + 7] << 8 | sprite[idx + 8] << 16 | sprite[idx + 9] << 24 | sprite[idx + 10] << 32 | sprite[idx + 11] << 40 )
        ...
        for (let i = 0; i < 16; i++, da >>= 3, dc >>= 2) {
          ...
          if (x < width && y < height) { // Beware of last column & row overflow
            const offset = (y * width + x) * 4;
            imgData.data[offset]     = color.r;
            imgData.data[offset + 1] = color.g;
            imgData.data[offset + 2] = color.b;
